...
pub const KITTY_DISAMBIGUATE: u5 = 1;
pub const KITTY_EVENT_TYPES: u5 = 2;
pub const KITTY_ALL_KEYS: u5 = 8;

pub fn encodeKey(event: KeyEvent, enc_state: EncoderState, out: *[128]u8) []const u8 {
    if (enc_state.kitty_flags != 0) return encodeKitty(event, enc_state, out);
...
fn encodeXterm(event: KeyEvent, enc_state: EncoderState, out: *[128]u8) []const u8 {
    ...
    switch (event.key) {
        .tab => {
            if (event.mods.shift) return writeStr(out, "\x1b[Z");
            return writeStr(out, "\t");
        },
        .enter => return writeStr(out, "\r"),
        .backspace => {
            if (event.mods.alt) return writeStr(out, "\x1b\x7f");
            return writeStr(out, "\x7f");
        },
        .escape => return writeStr(out, "\x1b"),
        .up, .down, .left, .right => return encodeArrow(event.key, event.mods, enc_state.cursor_keys_app, out),
        ...
        .kp_0, .kp_1, .kp_2, .kp_3, .kp_4, .kp_5, .kp_6, .kp_7, .kp_8, .kp_9, .kp_decimal, .kp_divide, .kp_multiply, .kp_minus, .kp_plus, .kp_enter, .kp_equal => return encodeNumpad(event.key, event.mods, enc_state.keypad_app_mode, out),
        .codepoint => return encodeCodepoint(event.codepoint, event.mods, out),
    }
}
...
fn encodeArrow(key: KeyCode, mods: Modifiers, app_mode: bool, out: *[128]u8) []const u8 {
    ...
    if (mods.any()) return bufPrint(out, "\x1b[1;{d}{c}", .{ mods.toCSI(), letter });
    if (app_mode) {
        out[0] = 0x1b;
        out[1] = 'O';
        out[2] = letter;
...
fn encodeFKey5to12(key: KeyCode, mods: Modifiers, out: *[128]u8) []const u8 {
    ...
    if (mods.any()) return bufPrint(out, "\x1b[{d};{d}~", .{ code, mods.toCSI() });
...
fn encodeKitty(event: KeyEvent, enc_state: EncoderState, out: *[128]u8) []const u8 {
    ...
    if (flags & KITTY_EVENT_TYPES == 0 and event.event_type != .press) return out[0..0];
    if (flags & KITTY_ALL_KEYS != 0) return encodeKittyCSIu(event, enc_state, out);
    ...
    if (flags & KITTY_DISAMBIGUATE != 0) {
        switch (event.key) {
            ...
            .f5, .f6, .f7, .f8, .f9, .f10, .f11, .f12 => return encodeFKey5to12(event.key, event.mods, out),
            .page_up, .page_down, .insert, .delete => return encodeTildeKey(event.key, event.mods, out),
            else => return encodeKittyCSIu(event, enc_state, out),
        }
    }

    if (event.event_type != .press) return encodeKittyCSIu(event, enc_state, out);
    return encodeXterm(event, enc_state, out);
}

fn encodeKittyCSIu(event: KeyEvent, enc_state: EncoderState, out: *[128]u8) []const u8 {
    const mod_val = event.mods.toCSI();
    const need_event = (enc_state.kitty_flags & KITTY_EVENT_TYPES != 0) and event.event_type != .press;
    const need_mods = mod_val > 1 or need_event;
    const cp = kittyCodepoint(event);

    if (need_mods) {
        if (need_event) return bufPrint(out, "\x1b[{d};{d}:{d}u", .{ cp, mod_val, @intFromEnum(event.event_type) });
